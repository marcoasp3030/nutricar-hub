    ...
    try {
      ...
      if (action === 'produtos') {
        ...
        const [topVenda, menosVenda, porHora, porCategoria, margemProduto, porDiaSemana, totalProdutos, categoriaPorHora, topPorCategoria] = await Promise.all([
          sql.unsafe(`SELECT produto as name, COALESCE(SUM(quantidade::numeric),0) as quantidade, COALESCE(SUM(valor::numeric),0) as valor, COALESCE(SUM(valor_compra::numeric),0) as valor_compra FROM ${tableNameClean} ${whereOk} GROUP BY produto ORDER BY valor DESC LIMIT 15`),
          sql.unsafe(`SELECT produto as name, COALESCE(SUM(quantidade::numeric),0) as quantidade, COALESCE(SUM(valor::numeric),0) as valor FROM ${tableNameClean} ${whereOk} GROUP BY produto HAVING SUM(valor::numeric) > 0 ORDER BY valor ASC LIMIT 15`),
